...
class RequestRestEmailView(View):
    ...
    def post(self,request):
        ...
        if user.exists():
            ...
            EmailThread(email_message).start()
            messages.info(request,"WE HAVE SENT YOU AN EAMIL WITH INSTRUCTION ON HOW TO RESERT THE PASSWORD")
            return render(request,'auth/request-reset-email.html')
...
class SetNewPasswordView(View):
            def get(self,request,uidb64,token):
                ...
                try:
                    ...
                    if not PasswordResetTokenGenerator().check_token(user,token):
                        messages.warning(request,"Passsword Reset Link is Invalid")
            ...
            def post(self,request,uidb64,token):
                ...
                try:
                    ...
                    user.save()
                    messages.success(request,"password Rset Success Please Login With New Password")
                    return redirect('/e_auth/login/')  
                except DjangoUnicodeDecodeError as identifier:
